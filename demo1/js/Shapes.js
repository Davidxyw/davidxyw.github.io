...
Shape.prototype.moveDown=function(){//当前图形下落一格
	...
	//遍历当前图形的每个格
	for(var i=0;i<this.cells.length;i++){ this.cells[i].r+="1;//" 将每个格的r+="1" }="" };="" shape.prototype.moveleft="function(){//当前图形左移一格" 遍历当前图形的每个格="" 将每个格的c-1="" for(var="" i="0;i<this.cells.length;i++){" this.cells[i].c-="1;" shape.prototype.moveright="function(){//当前图形右移一格" 将每个格的c+1="" this.cells[i].c+="1;" shape.prototype.rotater="function(){//shape.rotateR();" this-="">shape
	//将shape的statei+1
	this.statei+=1;
...
Shape.prototype.rotateL=function(){//shape.rotateL();
	...
	this.statei<0&&(this.statei=this.states.length-1); this.rotate();="" }="" shape.prototype.rotate="function(){" 获得参照格对象:="" cells[orgi],保存在cell中="" cell:{r:?,="" c:?}="" var="" cell="this.cells[this.orgi];" 获得新状态对象:="" states[statei],保存在state中="" state:{r0:?,c0:?,="" r1:?,c1:?,="" ...}="" state="this.states[this.statei];" 遍历cells中所有格对象="" for(var="" i="0;i<this.cells.length;i++){" 当前格的r="cell.r+state["r"+i]" this.cells[i].r="cell.r+state["r"+i];" 当前格的c="cell.c+state["c"+i]" this.cells[i].c="cell.c+state["c"+i];" 定义状态对象结构，描述所有状态。="" function="" state(r0,c0,r1,c1,r2,c2,r3,c3){="" this.r0="r0;" this.r1="r1;" this.r2="r2;" this.r3="r3;" this.c0="c0;" this.c1="c1;" this.c2="c2;" this.c3="c3;" 创建具体图形类型,继承shape类型结构以及原型对象：="" t(){="" 专门描述所有t型图形的数据结构="" shape.call(this,[="" this-="">正在创建的新对象
...
//Z (0,3, 0,4, 1,4, 1,5)   orgi:2   2个状态</0&&(this.statei=this.states.length-1);></this.cells.length;i++){>