...
var floor={
	...
	init:function(){
		...
		document.onscroll=function(){
			...
			//  遍历spans中每个span
			for(var i=0;i<spans.length;i++){ 获得当前span距页面顶部的top值="" var="" top="getElementTop(spans[i]);" 获得页面滚动的scrolltop值="" scrolltop="document.documentElement.scrollTop" ||document.body.scrolltop;="" 计算距文档显示区顶部top:top-scrolltop,保存在变量innertop中="" innertop="top-scrollTop;" 找到该气泡对应的li下所有a="" as="$("#elevator" li[data-idx=""+
						parseInt(spans[i].innerHTML)
						+"" ]="">a");
				//		如果innerTop>MINTOP&&<maxtop if(innertop="">me.MINTOP&&innerTop<me.maxtop){ 修改当前span的class为hover="" spans[i].classname="hover" ;="" 设置li下第1个a隐藏="" as[0].style.display="none" 设置li下第2个a显示="" as[1].style.display="block" }else{="" 否则="" 清除当前span的class属性="" 设置li下第1个a显示="" 设置li下第2个a隐藏="" }="" if(i="=0){//如果当前元素是第1个span" 如果top<="scrollTop+MAXTOP" if(top<="scrollTop+me.MAXTOP){" 将id为elevator的元素显示="" $("#elevator")[0].style.display="block" 将id为elevator的元素隐藏="" 找到id为elevator下的ul绑定onmouseover事件="" $("#elevator="">ul")[0].onmouseover=function(e){
...
);</me.maxtop){></maxtop></spans.length;i++){>